...
> different types: integers and floating point numbers (or floats).
>
>~~~{.python}
> weight_kg_int = 57
...
This statement doesn't produce any visible output. We can see the values in the object the variable `data` is assigned to using `print`:
...
The temperature data is in 10ths of a degree Celsius. Let's load it again and convert it to degrees Celsius by dividing by 10. To guard against hidden errors from integer division, we should write the denominator as a float just in case the data imports as integers:
